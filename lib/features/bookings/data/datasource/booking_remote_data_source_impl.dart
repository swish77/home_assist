...
class BookingRemoteDataSourceImpl implements BookingRemoteDataSource{
  ...
  Future<void> cancelBookings(String bookingId) async {
        try {
          ...
          await dio.patch('/booking/$bookingId',
              data: {
                'status': "Cancelled"
              });
        ...
        on DioException{
          throw Exception("Unable to cancel booking $bookingId");
        }

    }

  @override
  Future<void> rescheduleBooking({required String bookingId, required DateTime bookingDate, required TimeOfDay bookingTime}) async {
    try{

      print("booking id: $bookingId, date: ${bookingDate.toIso8601String().split('T').first}"
      ...
      await dio.patch('/booking/$bookingId',
            data: {
                ...
                'bookingTime' : '${bookingTime.hour.toString().padLeft(2, '0')}:'
                    '${bookingTime.minute.toString().padLeft(2, '0')}'
            });
    }
    on DioException{
      throw Exception("Unable to reschedule booking $bookingId");
    }
  }
